    ...
    computed: {
        ...
        relatedResourceColumns() {
            return [
                {
                    property: 'type',
                    label: this.$tc('easycredit-payment.transactionHistory.types.type'),
                    rawData: true
                },
                {
                    property: 'total',
                    label: this.$tc('easycredit-payment.transactionHistory.types.amount'),
                    rawData: true
                },
                {
                    property: 'id',
                    label: this.$tc('easycredit-payment.transactionHistory.types.trackingId'),
                    rawData: true
                },
                {
                    property: 'status',
                    label: this.$tc('easycredit-payment.transactionHistory.types.state'),
                    rawData: true
                },
                {
                    property: 'transactionFee',
                    label: this.$tc('easycredit-payment.transactionHistory.types.transactionFee'),
                    rawData: true
                },
                {
                    property: 'paymentMode',
                    label: this.$tc('easycredit-payment.transactionHistory.types.paymentMode'),
                    rawData: true
                },
                {
                    property: 'create',
                    label: this.$tc('easycredit-payment.transactionHistory.types.createTime'),
                    rawData: true
                },
                {
                    property: 'update',
                    label: this.$tc('easycredit-payment.transactionHistory.types.updateTime'),
                    rawData: true
                }
            ];
        }
    ...
    methods: {
        createdComponent() {
            ...
            this.orderStore.getByIdAsync(orderId).then((order) => {
                order.getAssociation('transactions').getList({ page: 1, limit: 1 }).then((orderTransactions) => {
                    const easycreditPaymentId = orderTransactions.items[0].customFields.swag_easycredit_transaction_id;
                    this.EasyCreditPaymentService.getPaymentDetails(easycreditPaymentId).then((payment) => {
                        ...
                        this.createNotificationError({
                            title: this.$tc('easycredit-payment.paymentDetails.error.title'),
        ...
        pushRelatedResource(type, relatedResource) {
            ...
            this.relatedResources.push({
                ...
                type: this.$tc(`easycredit-payment.transactionHistory.states.${type}`),
